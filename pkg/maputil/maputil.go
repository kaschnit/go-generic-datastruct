...
import "github.com/kaschnit/go-ds/pkg/containers/enumerable"
...
func Find[K comparable, V any](m map[K]V, predicate enumerable.Predicate[K, V]) (K, V, bool) {
	for key, value := range m {
		if predicate(key, value) {
...
func MapMap[K comparable, V any, KR comparable, VR comparable](
	m map[K]V, mapper enumerable.MapMapper[K, V, KR, VR],
) map[KR]VR {
